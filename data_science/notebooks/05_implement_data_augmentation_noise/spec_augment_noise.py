...
plt.colorbar(format='%+2.0f dB')
plt.show()
print(data_df.feature.iloc[i])
print(data_df.label.iloc[i])
display.specshow(data_augmentation.noise(data_df.feature.iloc[i]), x_axis='time', y_axis='mel')
plt.colorbar(format='%+2.0f dB')
plt.show()
print(data_augmentation.noise(data_df.feature.iloc[i]))
